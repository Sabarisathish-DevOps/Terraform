...
variable "asg_name" {
  type    = string
  default = "Trader Gateway AS"
}

variable "instance_key" {
  type    = string
  default = "/home/ubuntu/zebacus-prod.pem"
}
...
variable "ami_name" {
  type    = string
  default = "trader-gateway-ami"
}

variable "launch_template_name" {
  type    = string
  default = "trader-gateway-lt"
}
...
resource "null_resource" "update_source_reload_pm2" {
  ...
  provisioner "remote-exec" {
    inline = [
      "PROJECT_DIR=$(find / -type d -name 'trader-user-gateway-nest-js' -print -quit)",
      "cd $PROJECT_DIR",
      "git remote -v",
      "git pull origin rajesh/ico",
      "npm install",
      "npm run build",
      "pm2 reload 1",
